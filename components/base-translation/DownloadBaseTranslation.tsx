...
const DownloadBaseTranslation: React.FC = () => {
  const { baseTranslation, repositoryLanguage } = useBaseStore()

  /**
   * Descarga el archivo de traducción base
   * In: baseTranslation, repositoryLanguage
   * Out: Descarga del archivo
  **/
  const handleDownload = () => {
    if (!baseTranslation || !repositoryLanguage) return
  ...
  return (
    <div className='flex flex-col mt-8 gap-4'>
      ...
      <div className='flex gap-4'>
        {repositoryLanguage && (
          <div
            className='flex items-center gap-2 cursor-pointer border rounded-lg p-2 pr-3'
